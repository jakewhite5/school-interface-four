...
class SchoolInterface:
    ...
    def run(self):
        staff_id = input('Please enter your staff id number :')
        staff_password = input('Please enter your password :')
        if self.school.authenticate_staff_by_id(staff_id, staff_password) == False:
            print('Unrecognzed credentials. Please try again')
            self.run()
        while True:
